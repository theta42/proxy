...
## create invite token *post* `/users/invite`
```bash
...
## sing up *post* `/auth/invite/<INVITE TOKEN>`
```bash
...
## verify SSH key *post* `/auth/verifykey`
```bash
...
## add ssh key to current user *post* `/users/key`
```bash
